                    ...
                    <v-card color="#9F2A2A" outlined>
                        <v-list-item>
                            ...
                            <v-list-item-content class="mt-2 mb-2">
                                <v-list-item-title><h2>Editar Estructura de Crédito</h2></v-list-item-title>
                                <v-list-item-subtitle>Edite la información pertinente</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                    </v-card>
                </v-row>
...
const soloLetras = helpers.regex('alpha', /^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]*$/);
  ...
  export default {
    ...
    methods: {
      ...
      cargar () {
            try {
                axios.get("../../api/estructura/"+this.$route.params.id).then(({ data }) => (this.form = data))
