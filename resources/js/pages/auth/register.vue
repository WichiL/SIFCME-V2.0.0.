    ...
    <div class="col-lg-8 m-auto">
      <card  :title="$t('register')">
        <form @submit.prevent="register" @keydown="form.onKeydown($event)">
          ...
          <!-- Rol -->
          <div class="form-group row">
            <label class="col-md-3 col-form-label text-md-right">{{ $t('rol') }}</label>
            <div class="col-md-7">
              <select name="rol" v-model="form.rol" class="form-control" :class="{ 'is-invalid': form.errors.has('rol') }">
                <option value="" disabled selected >Seleccione un rol para el usuario</option>
                <option value="Admin">Admin</option>
                <option value="Promotor">Promotor</option>
                <option value="Solicitante">Solicitante</option>
              </select>
              <has-error :form="form" field="rol" />
            </div>  
          </div>
